...
def predict(x, y=None, model_gap=None, model=None, model_CNN=None, is_norm=False, PCA=False):
    '''
    ...
    if PCA:
        feat_MG = pca_transform(feat_MG, path='models/PCA/PCA_MultiGap_8464_auto.pkl')        
    if model_CNN:
        feat_CNN = model_CNN.predict(y, verbose=0)
        if is_norm:
            feat_CNN = normalize_feat_vector(feat_CNN)
        if PCA:
            feat_CNN = pca_transform(feat_CNN, path='models/PCA/PCA_CNN_1280_auto.pkl')
...
def predict_from_path(model_gap, model, paths, resize_func=None, size=None, for_all=False, save_results=None, save_to=None, model_CNN=None, is_norm=False, PCA=False):
    ...
    for i, path in enumerate(paths):
        ...
        pred_score = predict(img_mg, img_cnn, model_gap, model, model_CNN, is_norm, PCA)
        predicted.append(pred_score)
...
def plot_pred_orig(model_gap, model, imgs_bench, label=None, row_count=2, column_count=10, resize_func=None, size=None, for_all=False, model_CNN=None, is_norm=False, PCA=False):
